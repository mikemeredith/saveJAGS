...
as.mcmc.list.saveJAGSfileList <- function(x, params=NULL, thin=1, burnFiles=0, cores=1, ...) {
  ...
  if(cores < 2) {
    MC <- lapply(x, combineSaves1chain, wanted=wanted, thin=thin,
      burnFiles = burnFiles, fl=fl)
  } else {
    coresToUse <- min(cores, length(x), detectCores())
    cl <- makeCluster(coresToUse) ; on.exit(stopCluster(cl))
    MC <- parLapply(cl, x, combineSaves1chain, wanted=wanted, thin=thin,
      burnFiles = burnFiles, fl=fl)
  }
  # Check for errors
